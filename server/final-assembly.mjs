...
import { musicVideoScenes, musicVideoJobs } from '../drizzle/schema.ts';
...
const cinematic = scenes.filter(s => s.sceneType === 'cinematic').sort((a, b) => a.sceneIndex - b.sceneIndex);
...
// Total 11 scenes: perf0, cin1, perf2, cin3, perf4, cin5, perf6, cin7, perf8, cin9, cin10
const sceneOrder = [];
const cinematicByIndex = {};
for (const s of cinematic) {
  cinematicByIndex[s.sceneIndex] = s.videoUrl;
}

// Map: scene indices in the final video
// Performance: 0, 2, 4, 6, 8 (mapped from perfLipSyncUrls keys)
// Cinematic: 1, 3, 5, 7, 9, 10 (from job 660001 cinematic scenes)
for (let i = 0; i <= 10; i++) {
  if (i % 2 === 0 && i <= 8) {
    sceneOrder.push({ type: 'performance', idx: i, url: perfLipSyncUrls[i] });
  } else {
    // Find the cinematic scene - they may have different indices in the DB
    const cinScene = cinematic[Math.floor(i / 2)];
    if (cinScene) {
