...
export async function generateSoundEffect(params: {
  ...
  console.log(`[ElevenLabs SFX] Generating ${clampedDuration}s sound effect: "${params.prompt.substring(0, 80)}"`);
...
export async function generateMusic(params: {
  ...
  if (params.durationSeconds) {
    ...
    enrichedPrompt = `${durationLabel} track. ${params.prompt}`;
    console.log(`[ElevenLabs Music] Duration-enriched prompt: "${enrichedPrompt.substring(0, 120)}"`);
  }

  console.log(`[ElevenLabs Music] Submitting generation request...`);
  ...
  if (!generationId) {
    throw new Error(`ElevenLabs Music: no generation ID in response: ${JSON.stringify(submitResponse.data).substring(0, 200)}`);
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const statusResponse = await axios.get(
      `${BASE_URL}/text-to-music/${generationId}`,
      {
    ...
    if (status === "complete" || status === "completed") {
      ...
      throw new Error(`ElevenLabs Music generation failed: ${statusResponse.data.error || "unknown error"}`);
    }
