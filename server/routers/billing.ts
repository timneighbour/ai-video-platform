...
export const renderRouter = router({
  ...
  /**
   * Quick Preview for a render job — generates a free low-res 4-second draft of the first scene.
   * Runs in background; caller polls getRenderJobById for previewStatus/previewVideoUrl updates.
   */
  quickPreview: protectedProcedure
    .input(z.object({ renderJobId: z.number().int() }))
    .mutation(async ({ ctx, input }) => {
      const { getRenderJob, getDb } = await import("../db");
      const { TRPCError } = await import("@trpc/server");
      ...
      if (job.previewStatus === "generating") {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Preview already generating" });
      }
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database unavailable" });
      // Mark as generating
      await db.update(renderJobs)
        .set({ previewStatus: "generating" } as Partial<typeof renderJobs.$inferInsert>)
        .where(eq(renderJobs.id, input.renderJobId));
      // Fire-and-forget background render
      (async () => {
        try {
          let previewPrompt = "cinematic scene, smooth motion, 480p draft quality, 4 seconds";
          // Try to get a better prompt from the source job
          if (job.sourceJobId && job.sourceJobType === "music_video") {
            const { musicVideoScenes } = await import("../../drizzle/schema");
            const scenes = await db.select().from(musicVideoScenes)
              .where(eq(musicVideoScenes.jobId, job.sourceJobId!))
              .orderBy(musicVideoScenes.sceneIndex)
              .limit(1);
            if (scenes[0]?.prompt) {
              previewPrompt = scenes[0].prompt + ". smooth cinematic motion, 480p draft quality, 4 seconds";
            }
          } else if (job.sourceJobId && job.sourceJobType === "kids_video") {
            const { kidsVideoJobs } = await import("../../drizzle/schema");
            const [kidsJob] = await db.select().from(kidsVideoJobs)
              .where(eq(kidsVideoJobs.id, job.sourceJobId!))
              .limit(1);
            if (kidsJob?.storyPrompt) {
              previewPrompt = kidsJob.storyPrompt + ". Pixar 3D animation style, 480p draft quality, 4 seconds";
            }
          }
          const { generateFalSeedanceVideoSync } = await import("../ai-apis/fal-seedance");
          const result = await generateFalSeedanceVideoSync(
            { prompt: previewPrompt, aspect_ratio: "16:9", duration: "4", resolution: "480p", generate_audio: false },
            180_000
          );
          const resp = await fetch(result.videoUrl);
          const buf = Buffer.from(await resp.arrayBuffer());
          const key = `render-previews/${input.renderJobId}-preview-${Date.now()}.mp4`;
          const { url } = await storagePut(key, buf, "video/mp4");
          await db.update(renderJobs)
            .set({ previewStatus: "ready", previewVideoUrl: url } as Partial<typeof renderJobs.$inferInsert>)
            .where(eq(renderJobs.id, input.renderJobId));
          console.log(`[QuickPreview] Render job ${input.renderJobId} — preview ready: ${url}`);
        } catch (err) {
          console.error(`[QuickPreview] Render job ${input.renderJobId} failed:`, err);
          await db.update(renderJobs)
            .set({ previewStatus: "failed" } as Partial<typeof renderJobs.$inferInsert>)
            .where(eq(renderJobs.id, input.renderJobId));
        }
      })();
      return { started: true };
