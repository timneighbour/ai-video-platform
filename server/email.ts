...
/** Send an email to a specific customer (not just the owner) */
async function sendCustomerEmail(to: string, payload: EmailPayload): Promise<void> {
  const client = getResend();
  if (!client) return;

  try {
    const { error } = await client.emails.send({
      from: "WizVid AI <noreply@wizvid.ai>",
      to,
      subject: payload.subject,
      html: payload.html,
    });
    if (error) {
      ...
      console.log("[Email] Customer email sent to:", to);
    }
  } catch (err) {
...
/** Customer-facing: kids animation video is ready */
export async function emailKidsVideoComplete(data: {
  name: string;
  email: string;
  jobId: string;
  videoUrl: string;
...
<body style="margin:0;padding:0;background:#0f0f17;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
      ...
      <td style="background:#1a1a2e;border-radius:12px;overflow:hidden;border:1px solid #2d2d4e;">
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td style="padding:24px 24px 20px;background:linear-gradient(135deg,#1e1b4b,#312e81);">
              <span style="font-size:22px;font-weight:800;color:#fff;letter-spacing:-0.5px;">WizVid AI</span>
              <span style="display:block;font-size:13px;color:#a5b4fc;margin-top:4px;">Your Kids Animation is Ready! 🎉</span>
            </td>
          </tr>
        </table>
        <table width="100%" cellpadding="0" cellspacing="0" style="padding:16px 0;">
          <tr>
            <td style="padding:12px 24px;color:#f3f4f6;font-size:15px;">
              Hi ${data.name},<br/><br/>
              Your <strong>${data.style} animation</strong> (${data.videoLength}) has finished rendering and is ready to watch!
            </td>
          </tr>
          <tr>
            <td style="padding:8px 24px 20px;text-align:center;">
              <a href="${data.videoUrl}" style="display:inline-block;padding:14px 32px;background:linear-gradient(135deg,#7c3aed,#4f46e5);color:#fff;font-size:15px;font-weight:700;text-decoration:none;border-radius:8px;">
                Watch Your Animation
              </a>
            </td>
          </tr>
          <tr>
            <td style="padding:8px 24px 16px;color:#9ca3af;font-size:13px;">
              You can also log in to <a href="https://www.wizvid.ai" style="color:#a5b4fc;">wizvid.ai</a> and find your video under <strong>My Projects</strong>.
            </td>
          </tr>
        </table>
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td style="padding:12px 16px 20px;color:#6b7280;font-size:12px;border-top:1px solid #2d2d4e;">
              Job ID: ${data.jobId} · WizVid AI · wizvid.ai
            </td>
          </tr>
        </table>
      </td>
  ...
  await sendCustomerEmail(data.email, {
    subject: `🎉 Your Kids Animation is Ready — WizVid AI`,
    html,
  });
}
