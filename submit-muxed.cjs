...
async function submitLipSync(videoAssetId, audioAssetId, mode) {
  const body = JSON.stringify({
    title: `WizAI Scene 990015 ${mode}`,
...
async function main() {
  ...
  // Upload video WITH audio muxed in (so HeyGen Precision can replace it)
  console.log('Uploading muxed video (video+audio, 6s, 1280x720)...');
  const videoUp = await uploadToHeyGen('/tmp/s990015_with_audio.mp4', 'video/mp4');
  ...
  // Upload CBR MP3 clip (6s, 128kbps) as the replacement audio
  ...
  // Try precision first
  console.log('\nSubmitting with PRECISION mode...');
  const sub = await submitLipSync(videoId, audioId, 'precision');
  console.log('Submit status:', sub.status, JSON.stringify(sub.data || sub.raw).substring(0, 200));
