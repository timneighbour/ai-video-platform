/**
 * productContent.ts — Rich product-page content registry
 *
 * This is the SINGLE SOURCE OF TRUTH for all rich product-page copy:
 ...
 *   - benefits
 *   - keyFeatures
 *   - role / tagline (authoritative, mirrors products.ts)
 *
 * Relationship to client/src/lib/products.ts:
 *   products.ts   → lightweight identity registry (name, tagline, ctaLabel, routes)
 *                   consumed by nav, cards, grid, Create page
 *   productContent.ts → rich marketing copy (everything needed to render a full
 *                   product page via ProductPageTemplate)
 *                   consumed only by pages/products/index.tsx
 *
 * Visual/styling props (accentFrom, accentTo, accentGlow, borderColour, bgColour,
 * badgeClass, logo, heroImage) remain in pages/products/index.tsx because they are
 * presentation concerns, not content, and are not shared across any other surface.
 *
 * Image references (CARD_IMGS, HERO_IMGS) also remain in index.tsx for the same reason.
 */
...
export interface RichProductContent {
  /** Matches products.ts id */
  id: string;
  /** Canonical product name with ™ */
  name: string;
  /** Short role label, e.g. "The Brain" */
  role: string;
  /** One-line tagline — mirrors products.ts tagline */
  tagline: string;
  /** Hero headline */
  headline: string;
  /** Hero subheadline */
  subheadline: string;
  /** Full paragraph explaining what the product does */
  whatItDoes: string;
  /** Bullet-point capability list */
  capabilities: string[];
...
// ─── WizCreate ────────────────────────────────────────────────────────────────
const WIZCREATE_CONTENT: RichProductContent = {
  id: "wizcreate",
  name: "WizCreate™",
  role: "The Brain",
  ...
  subheadline:
    "WizCreate™ is the intelligence layer of WIZ AI — it transforms your audio track or text prompt into a fully-structured scene-by-scene visual plan, ready for animation and building.",
  whatItDoes:
    "WizCreate™ analyses your audio's tempo, mood, and lyrical content — or your text description — and generates a complete cinematic storyboard. Each scene is assigned a visual style, character placement, camera angle, and timing. It's the creative director that never runs out of ideas.",
  capabilities: [
...
// ─── WizAnimate ───────────────────────────────────────────────────────────────
const WIZANIMATE_CONTENT: RichProductContent = {
  id: "wizanimate",
  name: "WizAnimate™",
  role: "The Performer",
  ...
  subheadline:
    "WizAnimate™ takes your storyboard and breathes life into every character — generating fluid, beat-matched animation that looks like it was directed by a professional.",
  whatItDoes:
    "WizAnimate™ is the performance engine of WIZ AI. It takes the storyboard from WizCreate™ and generates character animation that is precisely matched to the audio — every movement, gesture, and expression timed to the beat, mood, and energy of the track.",
  capabilities: [
  ...
  keyFeatures: [
    { icon: "music", title: "Beat-Locked Motion Engine", desc: "Every character movement, camera cut, and scene transition is timed to the audio's beat grid — automatically, with zero manual keyframing." },
    { icon: "play-circle", title: "Cinematic Camera System", desc: "Dynamic camera moves — dolly, pan, zoom, orbit — are chosen per scene based on emotional context, not random selection." },
    { icon: "layers", title: "Multi-Character Choreography", desc: "Animate 2, 3, or more characters simultaneously with correct spatial relationships, eye contact, and interaction physics." },
    { icon: "eye", title: "Style-Adaptive Animation", desc: "Switches seamlessly between Cinematic, Anime, 3D, and Illustrated styles — each with its own motion language and physics rules." },
    { icon: "zap", title: "Emotion-Driven Performance", desc: "Facial expressions, body language, and gesture intensity are driven by the audio's emotional arc — not scripted manually." },
    { icon: "arrow-right-circle", title: "WizSync-Ready Export", desc: "Animated scenes pass directly to WizSync™ for lip sync and voice assignment — no re-processing or format conversion required." },
  ],
};

// ─── WizSound ─────────────────────────────────────────────────────────────────
const WIZSOUND_CONTENT: RichProductContent = {
  id: "wizsound",
  name: "WizSound™",
  role: "The Composer",
  ...
  subheadline:
    "WizSound™ transforms your audio into studio-grade cinematic sound — richer bass, clearer highs, and a fuller, more immersive presence that makes your video feel professional.",
  whatItDoes:
  ...
  howItWorks: [
    ...
    { num: "04", icon: "film", title: "Baked into video", desc: "The enhanced audio is baked into the final video build by WizGenesis™." },
  ],
  benefits: [
...
// ─── WizLumina ────────────────────────────────────────────────────────────────
const WIZLUMINA_CONTENT: RichProductContent = {
  id: "wizlumina",
  name: "WizLumina™",
  ...
  subheadline:
    "WizLumina™ applies cinematic colour grading, HDR tone mapping, and film-level sharpening to transform flat AI-generated video into rich, vivid, cinema-quality visuals.",
  whatItDoes:
    "WizLumina™ is WIZ AI's visual post-processing engine. It analyses each frame of your AI-generated video and applies a cinematic enhancement pipeline — colour grading, HDR tone mapping, sharpening, and 4K upscaling — that transforms the output from flat AI imagery into rich, film-quality visuals.",
  capabilities: [
  ...
  howItWorks: [
    ...
    { num: "04", icon: "star", title: "Cinema-quality output", desc: "The enhanced video is passed to WizGenesis™ for final building and export." },
  ],
  benefits: [
...
// ─── WizGenesis ───────────────────────────────────────────────────────────────
const WIZGENESIS_CONTENT: RichProductContent = {
  id: "wizgenesis",
  name: "WizGenesis™",
...
// ─── WizBoost ─────────────────────────────────────────────────────────────────
const WIZBOOST_CONTENT: RichProductContent = {
  id: "wizboost",
  name: "WizBoost™",
  role: "The Amplifier",
  ...
  subheadline:
    "WizBoost™ connects your finished video to real viewers, creators, and fans — amplifying your reach across platforms automatically and growing your audience while you create.",
  whatItDoes:
    "WizBoost™ is the distribution and growth layer of WIZ AI. After your video is rendered by WizGenesis™, WizBoost™ helps you reach your audience — connecting your content to the WIZ AI creator network, optimising it for platform algorithms, and tracking performance across channels.",
  capabilities: [
...
// ─── WizScript ───────────────────────────────────────────────────────────────
const WIZSCRIPT_CONTENT: RichProductContent = {
  id: "wizscript",
  name: "WizScript™",
  ...
  subheadline:
    "WizScript™ transforms your idea — a single sentence, a concept, a topic — into a scene-by-scene video script with dialogue, visual direction, and storyboard panels, ready for WizGenesis™ to build.",
  whatItDoes:
    "WizScript™ is the creative intelligence layer of WIZ AI. It takes your raw idea — a sentence, a topic, or a mood — and structures it into a complete, production-ready video script. Each scene is defined with visual direction, dialogue, and timing, giving WizGenesis™ everything it needs to build your video automatically.",
  capabilities: [
...
// ─── WizPilot ─────────────────────────────────────────────────────────────────
const WIZPILOT_CONTENT: RichProductContent = {
  id: "wizpilot",
  name: "WizPilot™",
  ...
  subheadline:
    "WizPilot™ is the intelligent automation layer of WIZ AI — it takes your idea and orchestrates every step of the creation pipeline automatically, from storyboard to final build, without a single manual handoff.",
  whatItDoes:
    "WizPilot™ is the automation intelligence of WIZ AI. You describe your vision — a track, a concept, a style — and WizPilot™ orchestrates the entire creation pipeline automatically. WizCreate™ builds the storyboard, WizAnimate™ generates the animation, WizSync™ locks the audio, WizSound™ enhances the mix, WizLumina™ grades the visuals, WizGenesis™ builds the final output. No manual steps. No handoffs. One prompt to finished video.",
  capabilities: [
  ...
  howItWorks: [
    ...
    { num: "03", icon: "cpu", title: "Pipeline orchestrated", desc: "On approval, WizPilot™ triggers the full pipeline: animation, audio sync, enhancement, visual grading, and final build — all automated." },
  ...
  keyFeatures: [
    { icon: "wand", title: "One-Prompt Video Creation", desc: "Describe your concept in plain language and WizPilot™ builds the entire video — storyboard, animation, audio, grading, and build — automatically." },
    ...
    { icon: "sparkles", title: "Free Storyboard Every Time", desc: "Every project starts with a free AI-generated storyboard. Regenerate unlimited times before committing credits to the final build." },
...
// ─── WizSync ──────────────────────────────────────────────────────────────────
const WIZSYNC_CONTENT: RichProductContent = {
  id: "wizsync",
  name: "WizSync™",
  role: "The Conductor",
  ...
  howItWorks: [
    ...
    { num: "04", icon: "film", title: "Sync map generated", desc: "A precise audio-visual sync map is generated and passed to WizAnimate™ and WizGenesis™ for beat-locked building." },
  ],
  benefits: [
  ...
  keyFeatures: [
    ...
    { icon: "arrow-right-circle", title: "Pipeline-Ready Sync Map", desc: "Generates a complete audio-visual sync map that passes directly into WizAnimate™ and WizGenesis™ — no manual export or timeline adjustment needed." },
  ],
};

// ─── WizScore ─────────────────────────────────────────────────────────────────
const WIZSCORE_CONTENT: RichProductContent = {
  id: "wizscore",
  name: "WizScore™",
  role: "The Composer",
...
// ─── WizShorts ────────────────────────────────────────────────────────────────
const WIZSHORTS_CONTENT: RichProductContent = {
  id: "wizshorts",
  name: "WizShorts™",
  ...
  subheadline:
    "WizShorts™ generates polished 9:16 short-form videos for YouTube Shorts, Instagram Reels, and TikTok — from a topic or script, in minutes.",
  whatItDoes:
    "WizShorts™ is WIZ AI's short-form video creation engine. It takes a topic, script, or concept and generates a complete 9:16 vertical video — with AI-generated scenes, optional music, and polished transitions — ready to publish on YouTube Shorts, Instagram Reels, or TikTok.",
  capabilities: [
...
// ─── WizVideo ─────────────────────────────────────────────────────────────────
const WIZVIDEO_CONTENT: RichProductContent = {
  id: "wizvideo",
  name: "WizVideo™",
  role: "The Director",
  tagline: "AI-Directed Music Video Production",
  headline: "You're the director. AI is your crew.",
  subheadline:
    "WizVideo™ is AI-directed music video production software. Upload your track, define your vision, and direct every scene — with phoneme-accurate lip sync, canonical character consistency, and lyric-aware visuals that understand what your song is about.",
  whatItDoes:
    "WizVideo™ gives you director-level control over every frame of your music video. The AI analyses your lyrics, emotional arc, and song structure to generate a scene-by-scene storyboard that reflects your vision. Add your performer, enable lip sync, direct each scene's camera angle and setting, and preview the result before you confirm a single download. This is AI performance capture — not random video generation.",
  capabilities: [
    "Phoneme-accurate lip sync — character mouth movements match the actual words sung",
    "Canonical character consistency — the same face across every scene, no drift",
    "Lyric-aware scene direction — visuals reflect what the song is about, line by line",
    "Emotional arc understanding — verse, chorus, bridge each get the right visual treatment",
    "Scene-level director controls — camera angle, setting, lip sync toggle per scene",
    "Quality guarantee — preview before you confirm, re-direct any scene for free before download",
  ],
  howItWorks: [
    { num: "01", icon: "upload", title: "Upload your track", desc: "Upload your audio file. WizVideo™ transcribes your lyrics and analyses the tempo, emotional arc, and song structure automatically." },
    { num: "02", icon: "cpu", title: "AI understands your song", desc: "Before generating a single scene, WizVideo™ analyses your lyrics, theme, and emotional arc — so every visual reflects what your song is actually about." },
    { num: "03", icon: "clapperboard", title: "Direct your scenes", desc: "Review every scene, edit prompts, choose camera angles, toggle lip sync, and re-direct any scene before rendering. You are the director." },
    { num: "04", icon: "film", title: "Preview, confirm & download", desc: "Watch your full video before confirming. Re-direct any scene for free. Once you're happy, confirm and download — no surprises, no wasted credits." },
  ],
  benefits: [
    { title: "Lip sync that actually works", desc: "Phoneme-level lip sync locks the performer's mouth movements to the exact words being sung — not a random approximation." },
    { title: "One face, every scene", desc: "Your performer's canonical portrait is generated once and used as the visual anchor for every scene — no random faces, no inconsistency." },
    { title: "Lyrics drive the visuals", desc: "The AI reads your lyrics and translates imagery, metaphors, and emotional tone into scene-by-scene visual direction — automatically." },
    { title: "Preview before you confirm", desc: "Watch your full video before committing. Re-direct any scene for free. Only download when you're satisfied." },
    { title: "Director-level scene control", desc: "Edit any scene's prompt, camera angle, setting, and lip sync toggle. Re-render just that scene — not the whole video." },
  ...
  keyFeatures: [
    ...
    { icon: "film", title: "AI Performance Capture", desc: "WizVideo™ captures a consistent AI performance — not just generated footage. The performer sings your song, in your setting, with your direction." },
  ],
};

// ─── WizPerformer ─────────────────────────────────────────────────────────────
const WIZPERFORMER_CONTENT: RichProductContent = {
  id: "wizperformer",
  name: "WizPerformer™",
  role: "The Face",
  ...
  subheadline:
    "WizPerformer™ uses a single reference photo to generate a consistent AI performer that appears across every scene of your video — with the same face, style, and presence throughout.",
  whatItDoes:
    "WizPerformer™ is WIZ AI's face-consistent character engine. Upload a single reference photo and WizPerformer™ generates a fully animated AI performer that maintains the same face, expression, and identity across every scene — no drift, no inconsistency, no manual work.",
  capabilities: [
...
const PRODUCT_CONTENT_REGISTRY: Record<string, RichProductContent> = {
  wizcreate:    WIZCREATE_CONTENT,
  wizanimate:   WIZANIMATE_CONTENT,
  wizsound:     WIZSOUND_CONTENT,
  wizlumina:    WIZLUMINA_CONTENT,
  wizgenesis:   WIZGENESIS_CONTENT,
  wizboost:     WIZBOOST_CONTENT,
  wizscript:    WIZSCRIPT_CONTENT,
  wizpilot:     WIZPILOT_CONTENT,
  wizsync:      WIZSYNC_CONTENT,
  wizscore:     WIZSCORE_CONTENT,
  wizshorts:    WIZSHORTS_CONTENT,
  wizvideo:     WIZVIDEO_CONTENT,
  wizperformer: WIZPERFORMER_CONTENT,
};

/** Returns the rich content for a product by id, or undefined if not found. */
export function getProductContent(id: string): RichProductContent | undefined {
  return PRODUCT_CONTENT_REGISTRY[id];
}

/** Returns all rich product content entries as an array. */
export function getAllProductContent(): RichProductContent[] {
  return Object.values(PRODUCT_CONTENT_REGISTRY);
}
