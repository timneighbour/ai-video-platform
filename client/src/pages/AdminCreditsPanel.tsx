...
export default function AdminCreditsPanel() {
  ...
  if (isLoading) return <div className="min-h-screen bg-background flex items-center justify-center"><div className="animate-spin w-8 h-8 rounded-full border-2 border-amber-500 border-t-transparent" /></div>;
  if (!user || user.role !== "admin") {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <Card className="bg-card border-white/10 max-w-sm w-full mx-4">
          <CardContent className="pt-6 text-center">
            <AlertCircle className="w-10 h-10 text-red-400 mx-auto mb-3" />
            <p className="text-white font-semibold mb-1">Admin Access Required</p>
            <p className="text-muted-foreground text-sm">This panel is restricted to administrators.</p>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      {/* Header */}
      <div className="sticky top-0 z-10 border-b border-white/10 backdrop-blur-md bg-background/90">
        ...
        <Tabs defaultValue="users">
          ...
          <TabsContent value="users">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                <Card className="bg-card border-white/10">
                  ...
                  <CardContent>
                    ...
                    {searching && <p className="text-sm text-muted-foreground/70 text-center py-4">Searching…</p>}

                    {searchResults && searchResults.length === 0 && (
                      <p className="text-sm text-muted-foreground/70 text-center py-4">No users found for "{activeQuery}"</p>
                    )}
                        ...
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                              ...
                              <Badge variant="outline" className="text-xs border-white/20 text-muted-foreground/70">{u.role}</Badge>
                            </div>
                          </div>
                        </button>
                      ))}
              ...
              <div>
                {!selectedUserId ? (
                  <Card className="bg-card border-white/10 h-full flex items-center justify-center">
                    <CardContent className="text-center py-12">
                      <User className="w-10 h-10 text-muted-foreground/50 mx-auto mb-3" />
                      <p className="text-muted-foreground/70 text-sm">Select a user to view their credits</p>
                    </CardContent>
                  </Card>
                ) : userCredits ? (
                  <Card className="bg-card border-white/10">
                      ...
                      <div className="grid grid-cols-3 gap-3 mb-4">
                        {[
                          ...
                          <div key={item.label} className="p-3 rounded-xl text-center bg-white/4 border border-white/8">
                            ...
                            <p className="text-xs text-muted-foreground/70 mt-0.5">{item.label}</p>
                          </div>
                        ))}
                      </div>

                      {/* Transaction history */}
                      <p className="text-xs text-muted-foreground/70 mb-2 font-semibold uppercase tracking-wider">Recent Transactions</p>
                      <div className="space-y-1.5 max-h-64 overflow-y-auto">
                        {userCredits.history.length === 0 && <p className="text-xs text-muted-foreground/50 text-center py-4">No transactions yet</p>}
                        {userCredits.history.map(tx => (
                          <div key={tx.id} className="flex items-center justify-between p-2.5 rounded-lg bg-white/3 border border-white/6">
                            <div className="min-w-0 flex-1">
                              <p className="text-xs text-foreground/80 truncate">{tx.description ?? tx.type}</p>
                              <p className="text-xs text-muted-foreground/50">{formatDate(tx.createdAt)}</p>
                            </div>
            ...
            {!disputes || disputes.length === 0 ? (
              <Card className="bg-card border-white/10">
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-muted-foreground">No {disputeStatus === "pending" ? "pending " : ""}disputes</p>
                </CardContent>
              </Card>
            ) : (
              <div className="space-y-3">
                {disputes.map(({ dispute, userName, userEmail }) => (
                  <Card key={dispute.id} className="bg-card border-white/10">
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-xs text-muted-foreground/70">#{dispute.id}</span>
                            <span className="text-xs text-muted-foreground/70">·</span>
                            <span className="text-xs text-muted-foreground/70">{formatDate(dispute.createdAt)}</span>
                          </div>
                          <p className="text-sm font-semibold text-white">{userName ?? userEmail ?? `User #${dispute.userId}`}</p>
                          <p className="text-xs text-muted-foreground/70 mb-2">{userEmail}</p>
                          ...
                          <p className="text-sm text-foreground/80 bg-white/4 rounded-lg p-3 border border-white/8">{dispute.reason}</p>
                          {dispute.adminNote && (
                            <p className="text-xs text-muted-foreground/70 mt-2 italic">Admin note: {dispute.adminNote}</p>
                          )}
                        </div>
      ...
      <Dialog open={adjustOpen} onOpenChange={setAdjustOpen}>
        <DialogContent className="bg-card border-white/10 text-white max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div>
              <label className="text-sm text-muted-foreground mb-1.5 block">Amount (positive = add, negative = deduct)</label>
              <Input
                type="number"
                value={adjustAmount}
              ...
              />
              <p className="text-xs text-muted-foreground/50 mt-1">Current balance: <strong className="text-amber-400">{userCredits?.credits.balance ?? 0} credits</strong></p>
            </div>
            <div>
              <label className="text-sm text-muted-foreground mb-1.5 block">Reason (required)</label>
              <Textarea
                value={adjustReason}
      ...
      <Dialog open={resolveOpen} onOpenChange={setResolveOpen}>
        <DialogContent className="bg-card border-white/10 text-white max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="flex gap-2">
              {(["approved", "partial", "rejected"] as const).map(type => (
                <button
                  ...
                  className={`flex-1 py-2 px-3 rounded-lg text-sm font-semibold border transition-colors ${resolveType === type ? "border-amber-500 bg-amber-500/15 text-amber-400" : "border-white/10 text-muted-foreground hover:bg-white/5"}`}
                >
            ...
            <div>
              <label className="text-sm text-muted-foreground mb-1.5 block">Credits to refund</label>
              <Input
                type="number"
                value={resolveAmount}
                ...
                className="bg-white/5 border-white/10 text-white"
              />
              <p className="text-xs text-muted-foreground/50 mt-1">Credits originally charged: <strong className="text-white">{resolveDispute?.creditsCharged ?? 0}</strong></p>
            </div>
            <div>
              <label className="text-sm text-muted-foreground mb-1.5 block">Admin note (optional)</label>
              <Textarea
                value={resolveNote}
