    ...
    >
      ...
      <div className="absolute top-3 right-3 z-10 pointer-events-none">
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full backdrop-blur-sm border"
          ...
          <img src={WIZLUMINA_LOGO} alt="WizLumina" className="w-3 h-3 rounded-full" />
          ...
          {(["standard", "enhanced", "cinematic"] as VisualMode[]).map((m) => {
            ...
            return (
              <button
                ...
                aria-pressed={active}
              >
                {m === "standard" ? <Eye className="w-3.5 h-3.5" /> : <img src={WIZLUMINA_LOGO} alt="WizLumina" className="w-3.5 h-3.5 rounded-full" />}
                {c.label}
              </button>
            );
          })}
