/**
 * Beauty of the Wreckage V3 — Resume Generation from S03
 * Skips S01 and S02 which are already completed.
 */
...
const LOG_FILE = "/tmp/botw-v3-resume.log";
...
async function generateScene(scene: Scene): Promise<void> {
  ...
  if (fs.existsSync(outputPath)) {
    ...
    if (stats.size > 500000) {
      log(`[SKIP] ${scene.id} already exists (${(stats.size / 1024 / 1024).toFixed(1)}MB)`);
      return;
    }
  }
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      if (result.status === "completed" && result.video_url) {
        log(`[COMPLETED] ${scene.id} — downloading...`);
        ...
        log(`[POLLING] ${scene.id} — ${result.status} (${attempts}/${maxAttempts})`);
      }
    } catch (pollErr: any) {
      if (pollErr.message?.includes("Generation failed")) throw pollErr;
      log(`[POLL_ERROR] ${scene.id} — ${pollErr.message}`);
    }
  }
  
  throw new Error(`Timeout: ${scene.id}`);
}
...
async function main() {
  ...
  for (const scene of scenes) {
    try {
      ...
      log(`[SKIPPING] ${scene.id} — ${err.message}`);
    }
  ...
  for (const r of results) {
    log(`  ${r.id}: ${r.success ? "✓" : `✗ ${r.error}`}`);
  }
  
  const succeeded = results.filter(r => r.success).length;
  log(`${succeeded}/${scenes.length} scenes completed`);
}
