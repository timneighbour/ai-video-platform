...
const AUDIO_URL = 'https://files.manuscdn.com/user_upload_by_module/session_file/310519663500868908/ZYxGLBgCfjjEERtg.mp3';
...
(async () => {
  console.log('[SyncLabs] Submitting: 6s Zara video + full mix audio (0-15s)');
  ...
  while (Date.now() < deadline) {
    ...
    if (s.status === 'COMPLETED') {
      ...
      const s3Url = await storagePut(`test-lipsync/synclabs-fullmix-${Date.now()}.mp4`, buf, 'video/mp4');
      console.log(`\n✅ RESULT: ${s3Url}`);
      return;
    }
    if (s.status === 'FAILED' || s.status === 'REJECTED') {
      throw new Error(`Job ${s.status}: ${s.error || JSON.stringify(s)}`);
    }
  }
